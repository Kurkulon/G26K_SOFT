...
static void ReqReadPHY(byte PhyReg);
	...
	inline bool CheckBufferOwn(Buf_Desc &buf) { return buf.addr1 & OWNERSHIP_BIT; }
	inline void SetBufferOwn(Buf_Desc &buf) { buf.addr1 &= ~OWNERSHIP_BIT; }
#elif defined(CPU_XMC48)
	inline void EnableMDI() { /*HW::GMAC->NCR |= GMAC_MPE;*/ }
	inline void DisableMDI() { /*HW::GMAC->NCR &= ~GMAC_MPE;*/ }
...
static void RecieveFrame()
{
	...
	else
	{
		...
		{
			mb = (MEMB*)((byte*)buf.addr1 - sizeof(mb->next));

			EthHdr *eth = (EthHdr*)(buf.addr1);
...
static void rx_descr_init (void)
{
	...
	for (u32 i = 0; i < NUM_RX_BUF; i++)
	{
		...
		Rx_Desc[NUM_RX_BUF-1].addr1 |= 0x02; // Set the WRAP bit at the end of the list descriptor.
