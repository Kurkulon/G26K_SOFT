...
static u16 verMemDevice = VERSION;
...
Ptr<UNIBUF> CreateTestDspReq01()
{
	...
	rsp.CM.hdr.time += 1;
	rsp.CM.hdr.hallTime += 1;
	rsp.CM.hdr.motoCount += 1;
	rsp.CM.hdr.headCount += 1;
	rsp.CM.hdr.ax += 1;
	rsp.CM.hdr.ay += 1;
	rsp.CM.hdr.az += 1;
	rsp.CM.hdr.at += 1;
	...
	rq->dataLen = (sizeof(rsp.CM.hdr)/2 + 1500)*2;
...
//	if (count != 0)
	{
		...
		if (ptr.Valid())
		{
			...
			RequestFlashWrite(ptr, rsp->CM.hdr.rw);
